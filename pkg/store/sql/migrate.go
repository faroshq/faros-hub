...
func (s *Store) migrate(ctx context.Context, c *config.Database) error {
	...
	if c.Type == DatabaseTypePostgres {
		...
		if err := createFK(s.db, models.Membership{}, models.User{}, "user_id", "id", "CASCADE", "CASCADE"); err != nil {
			logger.Info("failed to add DB FK: %s", err)
		}
		if err := createFK(s.db, models.Membership{}, models.Workspace{}, "workspace_id", "id", "CASCADE", "CASCADE"); err != nil {
			logger.Info("failed to add DB FK: %s", err)
		}
	}
	return nil
