...
func (s *Store) subscribeChangesPostgres(ctx context.Context, callback func(event *models.Event) error) error {
	...
	_, err := s.pgxPool.Exec(ctx, "LISTEN application_updates")
	if err != nil {
...
func (s *Store) _notify(ctx context.Context, id, channel string, event models.EventType) {
	if s.db.Dialector.Name() == DatabaseTypeSqlite {
		s._notifySQLlite(ctx, id, channel, event)
	} else {
		s._notifyPostgres(ctx, id, channel, event)
	}
}

func (s *Store) _notifySQLlite(ctx context.Context, id, channel string, event models.EventType) {
	logger := klog.FromContext(ctx)
...
func (s *Store) _notifyPostgres(ctx context.Context, id, channel string, event models.EventType) {
	logger := klog.FromContext(ctx)
	if s.pgxPool == nil {
	...
	_, err = s.pgxPool.Exec(ctx, fmt.Sprintf("select pg_notify('%s', $1)", channel), string(bts))
	if err != nil {
