...
func (c *Controller) patchIfNeeded(ctx context.Context, old, obj *edgev1alpha1.Agent) error {
	...
	_, err = c.farosClientSet.Cluster(clusterName.Path()).EdgeV1alpha1().Agents(obj.Namespace).Patch(ctx, obj.Name, types.MergePatchType, patchBytes, metav1.PatchOptions{}, subresources...)
	return err
}
