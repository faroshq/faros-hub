...
func NewListener(log *logrus.Entry, client *http.Client, url string, header http.Header) (*Listener, error) {
	...
	for attempts := 5; attempts > 0; attempts-- {
		c, err = ln.dial()
		if err != nil {
			ln.log.Infof("Can not create control connection %v", err)
...
func (ln *Listener) run() {
		...
		for {
			...
			case msg := <-writec:
				if _, err := ln.sc.Write(msg); err != nil {
					ln.log.Printf("tunneler.Listener: error writing message to server: %v", err)
					ln.Close()
					return
				}
			}
	...
	for {
		...
		if err := json.Unmarshal(line, &msg); err != nil {
			ln.log.Printf("tunneler.Listener read invalid JSON: %q: %v", line, err)
			return
		}
		switch msg.Command {
		case "keep-alive":
...
func (ln *Listener) dial() (net.Conn, error) {
	connect := ln.url + "/connect"
	pr, pw := io.Pipe()
	req, err := http.NewRequest("GET", connect, pr)
	if err != nil {
		ln.log.Infof("Can not create request %v", err)
		return nil, err
	}
	req.Header = ln.header

	ln.log.Infof("Listener creating connection to %s", connect)
	res, err := ln.client.Do(req)
	if err != nil {
		ln.log.Infof("Can not connect to %s request %v, retry %d", connect, err)
		return nil, err
	}
	if res.StatusCode != 200 {
		ln.log.Infof("Status code %d on request %v, retry %d", res.StatusCode, connect)
...
func (ln *Listener) grabConn() {
	...
	c, err := ln.dial()
	if err != nil {
		ln.log.Infof("Can not create connection %v", err)
...
func (ln *Listener) Accept() (net.Conn, error) {
	...
	ln.log.Infof("Accept connection")
	return c, nil
}
