...
type UserClusterInterface interface {
	Cluster(logicalcluster.Path) tenancyv1alpha1client.UserInterface
...
func (c *usersClusterInterface) Cluster(clusterPath logicalcluster.Path) tenancyv1alpha1client.UserInterface {
	if clusterPath == logicalcluster.Wildcard {
		panic("A specific cluster must be provided when scoping, not the wildcard.")
	}

	return c.clientCache.ClusterOrDie(clusterPath).Users()
}
